...
/**
 * This class represents a database of vehicle registrations.
 *
 * @author Your Name
**/
public class Database {
    ...
    public void mergeSort() {
        ...
        /* TODO 1: Implement a for loop that sets the size of the subarrays. */
        ...
        for (int subArrSize = 1; subArrSize < n; subArrSize *= 2) {
            ...
            /* TODO 5: Within the for loop Implement a nested for loop that creates and processes the subarrays */
            /* TODO 6: with each iteration, shift the left start index of the subarrays.*/
            /* TODO 7: Stop the for loop when all sub arrays have been processed. */
            for (int leftStart = 0; leftStart < n - subArrSize; leftStart += 2 * subArrSize) {
                ...
                /* TODO 9: Using the left, right, and right end indexes create a set of left and right subarrays */
                ...
                /* TODO 12: With each iteration, pass the merge index, which is the
                            left start index, and the two subarrays to the merge( ) method. */
    ...
    private void merge(int mergeIndex, ArrayList leftArray, ArrayList rightArray) {
        ...
        /* TODO 14: Implement the first while loop, which iterates over the elements in the subarrays. */
        /* TODO 15: Set the condition of the while loop to stop when
                    either all the left or right elements have been merged.*/
        while (left_Indx < leftArray.size() && right_Indx < rightArray.size()) {
            /* TODO 16: Within the first while loop, store the elements from the
                        left and right sub arrays in local variables according to their corresponding indexes.  */
            leftVal = (Registration) leftArray.get(left_Indx);
            rightVal = (Registration) rightArray.get(right_Indx);

            /* TODO 17: Uncomment the next two lines below and run the application. */
            ...
            if (leftVal.license.compareTo(rightVal.license) <= 0) {
                /* TODO 20: If true, merge the element from the left subarray into
                            the registration array and increment the left subarray index.   */
                this.registrations.set(mergeIndex, leftVal);
                left_Indx++;
            } else {
                /* TODO 21: Else, merge the element from the right subarray into
                            the registration array and increment the right subarray index.  */
        ...
        /* TODO 23: Implement a second while loop to merge any elements
                    from the left subarray that have not already been merged. */
        while (left_Indx < leftArray.size()) {
            /* TODO 24: Inside the second while loop, with each iteration,
                        obtain the value at the left index from the left subarray */
            leftVal = (Registration) leftArray.get(left_Indx);
        ...
        /* TODO 27: Implement a third while loop to merge any elements
                    from the right subarray that have not already been merged. */
        while (right_Indx < rightArray.size()) {
            /* TODO 28: Inside the second while loop, with each iteration
                        obtain the value at the right index from the right subarray */
            rightVal = (Registration) rightArray.get(right_Indx);
